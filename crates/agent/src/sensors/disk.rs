...
/// statvfs one mount (via libc): (total_bytes, avail_bytes, total_inodes, free_inodes).
/// f_bavail (unprivileged-available) counts reserved blocks as used — matches
/// what the agent's non-root user can actually see.
fn stat_mount(mount_point: &str) -> Option<(u64, u64, u64, u64)> {
...
pub fn disk_events(p: &ProcFs, cfg: &Config, ts: i64, host_id: &str) -> Vec<AgentEvent> {
    ...
    for m in &mounts {
        let Some((total, free, files, ffree)) = stat_mount(&m.mount_point) else {
            continue;
        };
